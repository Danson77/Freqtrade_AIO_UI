...
class NS4_dca(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ---- MINIMAL GUARD: missing informative data ----
        if informative is None or informative.empty:
            # Create columns that later code expects after merge_informative_pair
            # Use NaN so entry rules depending on _1h columns will evaluate False safely.
            for col in [
                'ha_close', 'rocr', 'rsi_14', 'cmf', 'sup_level', 'roc', 'r_480',
                'bb_lowerband2', 'bb_middleband2', 'bb_upperband2', 'bb_width',
                'r_84', 'cti_40'
            ]:
                informative[col] = np.nan

            # Ensure merge will work (needs index + OHLCV columns if empty)
            # Just merge an empty informative frame (will forward-fill nothing)
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    pmx = np.where(pm_arr > 0.0, np.where(mavalue < pm_arr, 'down', 'up'), '')

...
########################################################################################################################################################
    ...
    stoploss = -0.99 #-0.322
    trailing_stop = True
    trailing_stop_positive = 0.005           # Offest from profit for trailing stop.
    trailing_stop_positive_offset = 0.03     # Profit necessary to have trailing_stop_positive apply.
...
########################################################################################################################################################
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Condition 1: EWO above high threshold
        condition_1 = (
        ...
        # Condition 2: EWO below low threshold
        condition_2 = (
            (dataframe['close'] < (dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value)) &
        ...
        # Assign signals for Condition 1
        dataframe.loc[condition_1, 'enter_long'] = 1
        dataframe.loc[condition_1, 'enter_tag'] = 'buy_condition_1'

        # Assign signals for Condition 2
        dataframe.loc[condition_2, 'enter_long'] = 1
        dataframe.loc[condition_2, 'enter_tag'] = 'buy_condition_2'
...
########################################################################################################################################################
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if conditions:
            condition_combined = reduce(lambda x, y: x | y, conditions)
            dataframe.loc[condition_combined, 'exit_long'] = 1
            dataframe.loc[condition_combined, 'exit_tag'] = 'sell_1'

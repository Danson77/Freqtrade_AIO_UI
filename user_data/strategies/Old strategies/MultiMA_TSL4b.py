...
from freqtrade.strategy import (merge_informative_pair, CategoricalParameter, DecimalParameter, IntParameter, BooleanParameter, timeframe_to_minutes)
...
def vidya(dataframe, length=10, alpha=0.2):
    ...
    # Calculate True Range (TR) and Volatility
    tr = ta.TRANGE(dataframe['high'], dataframe['low'], dataframe['close'])
    volatility = ta.SMA(tr, timeperiod=length)
    
    # Calculate VIDYA
    diff = dataframe['close'] - ta.EMA(dataframe['close'], timeperiod=length)
    vidya = ta.EMA(diff, timeperiod=length, alpha=alpha)
    
    # Apply smoothing to VIDYA based on the volatility
    vidya = vidya * (1 + alpha * (volatility / ta.SMA(dataframe['close'], timeperiod=length)))
    
    vidya = vidya.fillna(0)  # Handle any NaN values
    return vidya
########################################################################################################################################################
# 
########################################################################################################################################################

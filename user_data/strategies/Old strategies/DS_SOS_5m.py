...
########################################################################################################################################################
    ...
    stoploss = -0.15
    trailing_stop = True  # Whether to use a trailing stop.
...
######################################################################################################################################################## 
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Initialize 'buy_tag' as an object type column if it does not exist
        if 'buy_tag' not in dataframe.columns:
            dataframe['buy_tag'] = np.nan  # Initialize the column with NaNs
            dataframe['buy_tag'] = dataframe['buy_tag'].astype(object)  # Ensure it's of type 'object'
...
######################################################################################################################################################## 
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if conditions:
            ...
            dataframe.loc[condition_combined, 'exit_tag'] = 'sell_stoploss_deadfish'

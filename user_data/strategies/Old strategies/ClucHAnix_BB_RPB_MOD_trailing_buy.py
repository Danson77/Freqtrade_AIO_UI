...
###########################################################################
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Replace ZEMA with DEMA
        dataframe['dema_30'] = ta.DEMA(dataframe['close'], timeperiod=30)  # 30-period DEMA
        dataframe['dema_200'] = ta.DEMA(dataframe['close'], timeperiod=200)  # 200-period DEMA

        # Update pump_strength calculation
                ...
                if(len(dataframe) >= 1):
                    ...
                    if trailing_buy['allow_trailing']:
                        if (not trailing_buy['trailing_buy_order_started'] and (last_candle['buy'] == 1)):
                            ...
                            # self.custom_info_trail_buy[pair]['trailing_buy']['enter_tag'] = f"initial_buy_tag (strat trail price {last_candle['close']})"
